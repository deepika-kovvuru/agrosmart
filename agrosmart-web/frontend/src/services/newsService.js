    ...
    return [
      {
        ...
        summary: 'The Cabinet Committee on Economic Affairs has announced Minimum Support Prices for major Kharif crops with a significant hike.',
      ...
      },
      {
        ...
        summary: 'Agricultural departments issued high-alert advisories following confirmed infestations across 4 districts.',
...
export const getFarmingTips = async (category = '') => {
  try {
    const query = category ? `?category=${encodeURIComponent(category)}` : '';
    return await apiFetch(`/farming_tips${query}`);
  } catch (_) {
    return [
      {
        icon: '🌱',
        title: 'Seed Treatment',
        description: 'Treat seeds with Trichoderma viride @ 4g/kg before sowing to prevent root rot & wilt.',
        tag: 'Pre-Sowing',
        category: 'Sowing'
      },
      {
